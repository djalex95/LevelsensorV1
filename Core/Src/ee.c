...
#if defined(STM32G070xx) || defined(STM32G0B1xx)
#define   _EE_SIZE              2048
#define   _EE_ADDR_INUSE        (((uint32_t)0x08000000) | (_EE_SIZE * _EE_USE_FLASH_PAGE_OR_SECTOR))
#define   _EE_PAGE_OR_SECTOR    PAGE_NUM
#if (_EE_USE_FLASH_PAGE_OR_SECTOR > 63)
...
bool ee_write(uint32_t startVirtualAddress, uint32_t len, uint8_t *data)
{
  ...
  for (uint32_t i = 0; i < len; i += 8)
  {
	...
	thisArray.arr_un[0] = data[i+0];
	thisArray.arr_un[1] = data[i+1];
	thisArray.arr_un[2] = data[i+2];
	thisArray.arr_un[3] = data[i+3];
	thisArray.arr_un[4] = data[i+4];
	thisArray.arr_un[5] = data[i+5];
	thisArray.arr_un[6] = data[i+6];
	thisArray.arr_un[7] = data[i+7];
	/*
    ...
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, ((i + startVirtualAddress)) + _EE_ADDR_INUSE, thisArray.val_un) != HAL_OK)
    {
      HAL_FLASH_Lock();
#if _EE_ICACHE_CTRL == 1
      HAL_ICACHE_Enable();
